...
parser.add_argument('-k', nargs='+', help='List of hyperparams for k', required=True)
...
out_file = os.path.join('..', args.bat + '_hpc', args.type + '.sh')
...
def generate_trainstring(v):
    res_str = "python3 ../tf/examples/EMI-RNN/step2_emi_fastrgnn_disp_det.py -O " + str(args.O)\
        ...
        v[3]) + " -Dat " + args.base\
              + " -rnd " + str(args.rnd) + " -it " + str(args.it) + " -ep " + str(args.ep) + " -ots " + str(args.ots) + " -k " + str(v[4]) + " -out $outname"

    return res_str

pool = ThreadPool()
hyperparams = np.asarray(list(itertools.product(p0, p1, p2, p3, p4))).tolist()
