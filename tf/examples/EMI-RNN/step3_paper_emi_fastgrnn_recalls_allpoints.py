...
# Stack training, val and test data
x_all = np.vstack((x_train, x_val, x_test))
y_all = np.vstack((y_train, y_val, y_test))
BAG_ALL = np.vstack((np.reshape(BAG_TRAIN,[-1,1]), np.reshape(BAG_VAL,[-1,1]), np.reshape(BAG_TEST,[-1,1])))
BAG_ALL=np.reshape(BAG_ALL, [-1])

emiDriver.loadSavedGraphToNewSession(modelPrefix, globalStep, redirFile=devnull)
predictions, predictionStep = emiDriver.getInstancePredictions(x_all, y_all, earlyPolicy_minProb,
                                                               minProb=0.99, keep_prob=1.0)

bagPredictions = emiDriver.getBagPredictions(predictions, minSubsequenceLen=k, numClass=NUM_OUTPUT)
print("Round: %2d, window length: %3d, Validation accuracy: %.4f" % (round_, ORIGINAL_NUM_TIMESTEPS, acc), end='')
print(', All-point Accuracy (k = %d): %f, ' % (k,  np.mean((bagPredictions == BAG_ALL).astype(int))), end='')

all_acc = np.mean((bagPredictions == BAG_ALL).astype(int))

# Print confusion matrix
print('\n')
bagcmatrix = utils.getConfusionMatrix(bagPredictions, BAG_ALL, NUM_OUTPUT)
